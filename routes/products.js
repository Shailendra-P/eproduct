...
res.redirect("/admin/adminlogin")
   ...
   pool.query("select P.*,(select U.unitvalue from units U where U.unitid=P.unitid) as unitname,(select PT.typename from producttypes PT where PT.typeid=P.producttypeid) as producttypename  from products P",function(error,result){
    ...
    else
    {
     ...
     if(result.length==0)
     {
        res.render("displayallproducts",{result:[],aresult:aresult,msg:'No Record Found'}) 
...
pool.query("insert into products(productname,productmodel,category,gst,producttypeid,unitid,currencytype,price,offertype,offerrate,stock,picture)values(?,?,?,?,?,?,?,?,?,?,?,?)",[req.body.productname,req.body.productmodel,req.body.category,req.body.gst,req.body.producttype,req.body.unit,req.body.currency,req.body.price,req.body.offer,req.body.offerrate,req.body.stock,req.filename],function(error,result){
        ...
        {  console.log(result)
            res.render("productinterface",{aresult:aresult,"message":"Record Submitted Sucessfully..."})
...
res.redirect("/admin/adminlogin")
 ...
 router.post("/updateproduct",function(req,res){
    ...
    if(req.body.btn=="Edit")
    {
        ...
        pool.query("update products set productname=?,productmodel=?,category=?,gst=?,producttypeid=?,unitid=?,currencytype=?,price=?,offertype=?,offerrate=?,stock=? where productid=?",[req.body.productname,req.body.productmodel,req.body.category,req.body.gst,req.body.producttype,req.body.unit,req.body.currency,req.body.price,req.body.offer,req.body.offerrate,req.body.stock,req.body.productid],function(error,result){
            if(error)
            {  console.log(error)
